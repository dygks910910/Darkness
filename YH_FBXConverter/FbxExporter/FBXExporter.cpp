...
bool FBXExporter::GetIndices(std::vector<int>& indices)
{
	if (mTriangleCount == 0)
		return false;
...
bool FBXExporter::GetVertex(std::vector<CModelClass::VertexType>& vertices)
{
	if (mVertices.size() == 0)
		return false;
	CModelClass::VertexType type;
	vertices.reserve(mVertices.size());
	for (unsigned int i = 0; i < mVertices.size(); ++i)
	{
		type.position.x = mVertices[i].mPosition.x;
		type.position.y = mVertices[i].mPosition.y;
		type.position.z = mVertices[i].mPosition.z;

		type.normal.x = mVertices[i].mNormal.x;
		type.normal.y = mVertices[i].mNormal.y;
		type.normal.z = mVertices[i].mNormal.z;

		type.texture.x = mVertices[i].mUV.x;
		type.texture.y = mVertices[i].mUV.y;
...
void FBXExporter::WriteAnimationToStream(std::ostream& inStream)
{
	...
	for (unsigned int i = 0; i < mSkeleton.mJoints.size(); ++i)
	{
		...
		FbxMatrix tmpMtx = out.Transpose();
		Utilities::WriteMatrix(inStream, tmpMtx, true);
		inStream << "\t\t</joint>\n";
	}
	inStream << "\t</skeleton>\n";
	inStream << "\t<animations>\n";
