...
void FBXImporter::ProcessSkeletonHierarchy(FbxNode* inRootNode)
{
...
void FBXImporter::ProcessSkeletonHierarchyRecursively(FbxNode* inNode, int inDepth, int myIndex, int inParentIndex)
{
...
void FBXImporter::ProcessJointsAndAnimations(FbxNode* inNode)
{
	FbxMesh* currMesh = inNode->GetMesh();
...
unsigned int FBXImporter::FindJointIndexUsingName(const std::string& inJointName)
{
...
void FBXImporter::ProcessMesh(FbxNode* inNode)
{
	FbxMesh* currMesh = inNode->GetMesh();
...
void FBXImporter::ReadUV(FbxMesh* inMesh, int inCtrlPointIndex, int inTextureUVIndex, int inUVLayer, XMFLOAT2& outUV)
{
...
void FBXImporter::ReadNormal(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, XMFLOAT3& outNormal)
{
...
void FBXImporter::ReadBinormal(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, XMFLOAT3& outBinormal)
{
...
void FBXImporter::ReadTangent(FbxMesh* inMesh, int inCtrlPointIndex, int inVertexCounter, XMFLOAT3& outTangent)
{
...
int FBXImporter::FindVertex(const PNTIWVertex& inTargetVertex, const std::vector<PNTIWVertex>& uniqueVertices)
{
...
/*
void FBXImporter::ReduceVertices()
{
CleanupFbxManager();
...
int FBXImporter::FindVertex(const Vertex::PNTVertex& inTarget, const std::vector<Vertex::PNTVertex>& inVertices)
{
int index = -1;
...
void FBXImporter::AssociateMaterialToMesh(FbxNode* inNode)
{
...
void FBXImporter::ProcessMaterials(FbxNode* inNode)
{
...
void FBXImporter::ProcessMaterialAttribute(FbxSurfaceMaterial* inMaterial, unsigned int inMaterialIndex)
{
	FbxDouble3 double3;
	FbxDouble double1;
...
void FBXImporter::ProcessMaterialTexture(FbxSurfaceMaterial* inMaterial, MaterialFBX* ioMaterial)
{
...
void FBXImporter::PrintMaterial()
{
...
void FBXImporter::PrintTriangles()
{
	for (unsigned int i = 0; i < mTriangles.size(); ++i)
	{
