...
void CFbxLoader::LoadFBX(const char * pFileName, std::vector<Vertex::Basic32>& vb, std::vector<UINT>& ib, CModel* model)
{

	std::vector<Vertex::Basic32>* vbtest = nullptr;
	int animcnt = 0;
	...
	if(!strcmp(pFileName , "idle.fbx"))
	{
		std::ifstream inanim("AnimationData/allanim.txt");
		std::ifstream intime("AnimationData/timedata.txt");
		for (int i = 0; i < 6; ++i)
		{
			int cnt = 0;
			//TestAnim();

			//mpScene->SetCurrentAnimationStack(mCurrentAnimationStack);

			//InitTime();

			//////////////////////////애니메이션 데이터 한번에 뽑기//////////////////////////////
			//int framecount = mStop.GetFrameCount(FbxTime::eFrames24) + 1;
			int framecount, animstart, animend;
			intime >> framecount >> animstart >> animend;
			//animstatenum[mAnimState] = framecount;
			//animationcount[mAnimState] = framecount;
			model->mAnimCount[animcnt] = framecount;
			vbtest = new std::vector<Vertex::Basic32>[framecount];

			//std::cout << mCurrentAnimationStack->GetName() << std::endl;
			for (int i = animstart; i <= animend; ++i)
			{
				//FbxTime currTime;
				//currTime.SetFrame(i, FbxTime::eFrames24);
				//DrawRecursive(mpScene->GetRootNode(), currTime, vb, ib);
				for (int j = 0; j < vb.size(); ++j)
					inanim >> vb[j].Pos.x >> vb[j].Pos.y >> vb[j].Pos.z;

				vbtest[cnt].resize(vb.size());
				vbtest[cnt] = vb;
				cnt++;
			}
			model->mAnimData.insert(std::pair<std::string, std::vector<Vertex::Basic32>*>(mAnimName[i], vbtest));
			//rad.insert(std::pair<std::string, std::vector<Vertex::Basic32>*>(mAnimName[i], vbtest));
			//mAnimState++;
			animcnt++;
		}
		inanim.close();
