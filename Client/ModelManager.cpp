	...
	IDXGISwapChain* swapChain, ID3D11DepthStencilState* tDepthDisableState)
{
	...
	for (int k = 0; k < ANIMCNT; ++k)
	{
		...
		/*	dc->IASetInputLayout(InputLayouts::PosTex);
			...
			mLoadingScene2.Initialize(device, 1500, 900, L"UITextures/loading3.png", 1500, 900);
			mLoadingScene2.Render(dc, 0, 0, true);
			HR(swapChain->Present(0, 0));
			mLoadingScene2.Shutdown();
		}
...
// 	dc->OMSetDepthStencilState(tDepthDisableState, 1);
	...
	mLoadingScene2.Initialize(device, 1500, 900, L"UITextures/loading4.png", 1500, 900);
	mLoadingScene2.Render(dc, 0, 0, true);
	HR(swapChain->Present(0, 0));
	mLoadingScene2.Shutdown();
	...
	mLoadingScene2.Initialize(device, 1500, 900, L"UITextures/loading5.png", 1500, 900);
	mLoadingScene2.Render(dc, 0, 0, true);
	HR(swapChain->Present(0, 0));
	mLoadingScene2.Shutdown();
	...
	else
	{
		...
		else
		{
			...
			if (GetAsyncKeyState('W') & 0x8000)
			{
				...
				if (!mSkinnedModelInstance[mMyId].mAnimOneCheck && mSkinnedModelInstance[mMyId].mKeyState != Keystate::run)
				{
					cs_packet_player_anmation_start* anim = reinterpret_cast<cs_packet_player_anmation_start*>(&send_buf);
					anim->size = sizeof(cs_packet_player_anmation_start);
					anim->type = CS_PACKET_START_ANIMATION;
