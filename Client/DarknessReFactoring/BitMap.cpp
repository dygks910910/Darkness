...
bool CBitMap::Initialize(WCHAR * textureFilename, int bitmapWidth, int bitmapHeight)
{
	bool result;


	// Store the screen size.
	m_screenWidth = mClientWidth;
	m_screenHeight = mClientHeight;
	...
	result = InitializeBuffers(md3dDevice);
	if (!result)
	{
		return false;
	}
...
bool CBitMap::Render(int positionX, int positionY)
{
	bool result;
	// Re-build the dynamic vertex buffer for rendering to possibly a different location on the screen.
	result = UpdateBuffers(md3dImmediateContext, positionX, positionY);
	if (!result)
	{
		return false;
	}
	// Put the vertex and index buffers on the graphics pipeline to prepare them for drawing.
	RenderBuffers(md3dImmediateContext);
...
bool CBitMap::LoadTexture(WCHAR * filename)
{
	bool result;
