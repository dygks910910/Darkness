...
void FBXExporter::ExportFBX( const float & scaleFactor)
{
	LARGE_INTEGER start;
	LARGE_INTEGER end;
	...
	WriteMeshToParameter(scaleFactor);
	
	CleanupFbxManager();
	std::cout << "\n\nExport Done!\n";
}

void FBXExporter::LoadFBX(const char* inFileName, GeometryGenerator::MeshData* mesh, const float& scaleFactor)
{
	Initialize();
	LoadScene(inFileName);
	ExportFBX(scaleFactor);
}
...
void FBXExporter::ProcessMaterialTexture(FbxSurfaceMaterial* inMaterial, myExporter::Material* ioMaterial)
{
...
void FBXExporter::WriteMeshToParameter(const float& scaleFactor)
{
	GeometryGenerator::Vertex vtx;
	for (unsigned int i = 0; i < mTriangleCount; ++i)
	{
		//ÀÎµ¦½º »ðÀÔ.
		mesh.Indices.push_back(mTriangles[i].mIndices[1]);
		mesh.Indices.push_back(mTriangles[i].mIndices[2]);
		mesh.Indices.push_back(mTriangles[i].mIndices[0]);
	}
	for (unsigned int i = 0; i < mVertices.size(); ++i)
	{
		//Á¤Á¡»ðÀÔ.
		vtx.Position = XMFLOAT3(
			mVertices[i].mPosition.x * scaleFactor,
			mVertices[i].mPosition.y * scaleFactor,
			mVertices[i].mPosition.z * scaleFactor
		);
		vtx.Normal = mVertices[i].mNormal;
		vtx.TexC = mVertices[i].mUV;
		vtx.TangentU = mVertices[i].mTangent;
