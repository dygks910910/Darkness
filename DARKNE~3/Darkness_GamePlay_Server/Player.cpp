...
void Player::Move(const int dir, const UINT id, BYTE* packet)
{
 	...
 	case CS_UP:
	{
		...
		XMVECTOR look =  XMVectorSet(player_move.camLook.x, player_move.camLook.y, player_move.camLook.z,0);
		look = XMVector3Normalize(look);
		XMStoreFloat3(&camLook, look);
		...
		// 움직인 플레이어와 집들간의 충돌
		// 6개의 집을 검사
		for (int i = 0; i < Room::GetInstance()->GetMaxObstacles(); ++i) {
			if (true == IntersectSphereOrientedBox(&objects[player_move.id]->GetSphereForObstacle(), &obstacles[i]->GetObb())) {
				...
				if (j == 50) {
					collCheck = true;
					cout << obstacles[i]->GetName() << endl;
					cout << objects[player_move.id]->GetWorldMatrix()._41 << endl;
					cout << objects[player_move.id]->GetWorldMatrix()._42 << endl;
					cout << objects[player_move.id]->GetWorldMatrix()._43 << endl;
					j = 0;
				}*/
				...
				if (1 == obstacles[i]->GetObbType()) {
					objWX = objects[player_move.id]->GetSphereForObstacle().Center.x;
	...
	/*if ( 50 == i) {
		...
		cout << "x" << objects[0]->GetWorldMatrix()._41 << endl;
		cout << "z" << objects[0]->GetWorldMatrix()._43 << endl;
